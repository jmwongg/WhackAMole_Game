const gameState = {//holds the game state as a global variable
  ...
  timeOutTimerHnd: -1,
  log: [],
  maxTrials: 60,//<=======UPDATE THIS TO 60 WHEN RUNNING EXPERIMENTS WITH PARTICIPANTS
  gameStart: -1,
  center: "",
  mole: "",
};
...
const userClickedMole = function () {
  ...
  const reactionTime = Date.now() - gameState.log[gameState.log.length - 1].rndDelayEnd;

  // Log the reaction time
  console.log("Reaction Time:", reactionTime);

  // Update the log with reaction time
  raiseEvent("userClickedMole", "logEvent", reactionTime);
  hideMole(true);
...
const handleLogEvents = function (evt) {
  ...
  switch (sequenceStep) {
    ...
    case "userClickedMole":
      ...
      // If the user clicked the mole, record the reaction time in the log
      const reactionTime = ts - lastEntry.rndDelayEnd;
      lastEntry.reactionTime = reactionTime;
      break;
    case "userMoveStart":
